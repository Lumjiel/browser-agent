...
# ask-deepseek.sh — 向 DeepSeek 提问并等待回答
# 用法: bash scripts/ask-deepseek.sh "<问题>" [timeout_seconds]
#
# 流程：输入问题 → 点击发送 → 等待回答完成（连续2次字数不变=结束）→ 输出
# 核心：用 eval + JS 拿最后一个有内容的 .ds-assistant-message-main-content
...
extract_text() {
    ...
    local payload
    if [ -n "$tab_id" ]; then
        payload=$(python3 -c "import json; print(json.dumps({'tabId':'$tab_id','action':'eval','timeout':10,'code':'''$code'''}))" 2>/dev/null)
    else
        payload=$(python3 -c "import json; print(json.dumps({'action':'eval','timeout':10,'code':'''$code'''}))" 2>/dev/null)
    fi
    
    curl -s -X POST "$API/interactive" \
        -H "Content-Type: application/json" \
        -d "$payload" \
        | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('result',{}).get('value',''))" 2>/dev/null
}

# ── 1. 获取 Tab ID ──
TAB=$(curl -s "$API/state" | python3 -c "
import sys,json
...
# ── 4. 点击发送 ──
curl -s -X POST "$API/interactive" -H "Content-Type: application/json" \
    -d "{\"tabId\":\"$TAB\",\"action\":\"click\",\"selector\":\"div.ds-button--primary.ds-button--filled.ds-button--circle\"}" > /dev/null

echo "已发送，等待回答..." >&2

# ── 5. 等待回答完成（连续2次字数不变 = 结束） ──
PREV_LEN=0
STABLE_COUNT=0
ELAPSED=0
POLL_INTERVAL=3
